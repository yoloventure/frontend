...
router.get('/:id', function (req, res, next) {
  YoloChatAd.findById({_id: req.params.id})
...
router.put('/:id', function (req, res, next) {
  ...
  YoloChatAd.findByIdAndUpdate({_id: req.params.id}, req.body).then(function () {
    //find and send back updated application for display
    chat.findOne({_id: req.params.id}, req.body).then(function (yoloChatAd) {
      res.send(yoloChatAd);
    });
  });
});
...
router.delete('/:id', function (req, res, next) {
  YoloChatAd.findByIdAndRemove({_id: req.params.id}).then(function (yoloChatAd) {
    res.send(yoloChatAd);
  });
});
