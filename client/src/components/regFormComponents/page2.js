...
class Page2 extends React.Component {
  ...
  handleSubmitAddress = (e) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        console.log(items);
        tempArray.push(
          <div className="row">Click on an address to submit it</div>
        );
        items.forEach((element) => {
          tempArray.push(
            <button onClick={(e) => this.selectAddress(e)}>
              {element.text}
            </button>
          );
  ...
  selectAddress = (e) => {
    ...
    console.log(
      "street:" + street + "city:" + cityState[0] + "cityState:" + cityState[1]
    );
  ...
  handleSubmitPage2Next = () => {
    ...
    if (!host.street || !host.city || !host.state) {
      this.setState({
        submittedPage2: true,
        errorPage2: "Please fill all required fields",
      });
    } else {
      this.props.setNextTrue();
    }
      ...
      <div className="">
        <form>
          <div className="container-fluid">
            ...
            <Button className="ml-3 mt-3" onClick={this.handleSubmitAddress}>
              Search For This Address
            </Button>
            <div>{this.state.addressResults}</div>
